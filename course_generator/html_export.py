...
def html_section_to_plain(section_html: str) -> str:
    text = re.sub(r"<script.*?>.*?</script>", " ", section_html, flags=re.IGNORECASE | re.DOTALL)
...
def build_full_course_markdown(
    ...
    """Extended Markdown export with lesson body text and quiz reference."""
    is_ru = language == "ru"
    parts = [build_markdown_summary(outline, docs_info, lesson_payloads, language)]

    parts.append(f"## {'Содержание уроков' if is_ru else 'Lesson content'}")
    lessons = outline.get("lessons", [])
    for idx, lesson in enumerate(lessons, start=1):
        title = str(lesson.get("title", f"Lesson {idx}")).strip()
        payload = lesson_payloads[idx - 1] if idx - 1 < len(lesson_payloads) else {}
        body = html_section_to_plain(str(payload.get("lesson_html", "")))
        ...
        if takeaways:
            parts.append(f"**{'Ключевые выводы' if is_ru else 'Key takeaways'}:**")
            parts.extend(f"- {t}" for t in takeaways if str(t).strip())
        parts.append(f"_{'режим' if is_ru else 'mode'}: {mode}_")
        parts.append("")

    if pretest_data or quiz_data:
        parts.append(f"## {'Тесты (справочно)' if is_ru else 'Quizzes (reference)'}")
        if pretest_data:
            parts.append(f"### {'Входной тест' if is_ru else 'Pre-test'}")
            for i, q in enumerate(pretest_data, start=1):
                parts.append(f"{i}. {q.get('question', '')}")
                for opt in q.get("options", []) or []:
                    parts.append(f"   - {opt}")
                parts.append(f"   **{'Ответ' if is_ru else 'Answer'}:** {q.get('correct_answer', '')}")
        if quiz_data:
            parts.append(f"### {'Итоговый тест' if is_ru else 'Final quiz'}")
    ...
    parts.append(f"\n---\n_Generated by Doc-to-Course Generator v{__version}_")
    return "\n".join(parts).strip() + "\n"
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    body.theme-dark .source-excerpt {{ background:#0f172a; }}
    body.theme-dark .quiz-results {{ background:#1e3a5f; color:#bfdbfe; }}
