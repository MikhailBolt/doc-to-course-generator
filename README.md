...
- **`--inspect-docs`** — document inventory without LLM
- **`--regenerate-fallback`** — retry thin/fallback lessons
- **`--min-quality-score`** — CI quality gate (exit code 2)
- HTML: **lesson progress** + **search** (saved in browser)
- **`run_manifest.json`** — compact run summary
- **`--from-bundle`** / **`--artifacts-only`** — rebuild from `course_bundle.json` without LLM
- **`--regenerate-lessons 2,4`** — refresh specific lessons only
- **`--validate-bundle`** — check bundle JSON
- **`pip install -e .`** — install CLI as `doc-to-course`
...
- **`--doctor`** — readiness check (Ollama, docs, FAISS, bundles)
- **`course_summary.json`** — compact integration-friendly export
- HTML: **keyboard navigation** (J/K, ↑/↓) and quiz **shuffle**
- **`--clean-output`** — prune old artifacts (`--keep-last`, `--yes`)
- HTML: **A+/A−** font size and **per-lesson notes** (saved in browser)
